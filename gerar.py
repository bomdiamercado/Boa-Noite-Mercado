#!/usr/bin/env python3
"""
Bom Dia Mercado - gerador de relatorio.
Le o texto do boletim (corpo da issue), pede ao Claude os campos em JSON
e encaixa no template modelo-completo.html.
"""
import os
import sys
import json
...
MODELO = "modelo-completo-noite.html"
SAIDA = "index.html"
...
Extraia as informacoes e devolva APENAS um objeto JSON valido: sem markdown,
sem crases, sem nenhum texto antes ou depois.

Chaves obrigatorias:
- DATA_LONGA: data por extenso, ex "Sexta-feira, 18 de setembro de 2026"
- DATA_CURTA: ex "18/09/2026"
- DESCRIPTION: manchete resumida em uma linha, ate 150 caracteres
- MANCHETE_TITULO: titulo curto da manchete, em MAIUSCULAS
- MANCHETE_TEXTO: paragrafo de abertura, 2 a 4 frases

PAINEL 1 (Primeira linha):
- DOLAR, IBOV, BRENT, TNOTE: valores de fechamento (so o numero, ex "5,1543")
- DOLAR_VAR, IBOV_VAR, BRENT_VAR, TNOTE_VAR: variacao, ex "+0,14%" ou "—"
- DOLAR_COR, IBOV_COR, BRENT_COR, TNOTE_COR: "pos" se subiu, "neg" se caiu, "neu" se estavel

PAINEL 2 (Segunda linha):
- TREASURY_2Y, CDS_BR, DXY, CDI: valores de fechamento (ex "3,52%", "120 pb", "98,42", "14,15%")
- TREASURY_2Y_VAR, CDS_BR_VAR, DXY_VAR, CDI_VAR: variacao (ex "↓ -2 pb", "↓ -1 pb", "↓ -0,18%", "↔ a.a.")
- TREASURY_2Y_COR, CDS_BR_COR, DXY_COR, CDI_COR: "pos" se bom, "neg" se ruim, "neu" se estavel
  (Nota: Treasury 2Y, CDS Brasil, DXY: "neg" se sobe, "pos" se cai; CDI: sempre "neu")
...
- TEXTO_JUROS, TEXTO_DOLAR, TEXTO_BOLSA, TEXTO_ELEICOES, TEXTO_PETROLEO,
  TEXTO_PETROBRAS: o texto corrido de cada secao, reescrito de forma fluida,
  um paragrafo cada. Se nao houver informacao para a secao, use
  "Nenhuma noticia relevante."
...
- Se nao houver informacao sobre algum indicador da segunda linha, use "—" como valor.
...
def main():
    print("Iniciando geracao do relatorio...")
